...
const empleadoGet = async(req, res = response) =>{
    const{cedula} = req.query //Desestructuracion
...
const empleadoPut = async(req, res = response) =>{
    ...
    try {const empleado = await Empleado.findOneAndUpdate({cedula: cedula},{nombre_Empleado:nombre_Empleado, correo:correo, direccion:direccion,telefono:telefono,estado_Empleado:estado_Empleado})
...
const empleadoDelete = async(req, res = response) =>{
    const { cedula} = req.body //catptura de atributos 
    let mensaje =''

    try {const empleado = await Empleado.deleteOne({cedula: cedula})
